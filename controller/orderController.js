...
exports.placeOrder = async (req, res) => {
    ...
    try {
        ...
        userCart.forEach(async (cart) => {
            ...
            await admin.Product.updateOne({ _id: cart.productId._id }, { $inc: { stock: -cart.quantity } })
        ...
        return res.status(200).json({ success: true, message: "Order success..", orderId: orderId })
    } catch (error) {
...
exports.orderConfirmationPage = async (req, res) => {
    const orderId = req.params.orderId;
    const orderDetails = await order.Order.findOne({ orderId }).populate('userId')
...
exports.orderCancel = async (req, res) => {
    try {
        ...
        orders.products.forEach(async (product) => {
            ...
            await admin.Product.updateOne({ _id: product.productId }, { $inc: { stock: product.quantity } })
